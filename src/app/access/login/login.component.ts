...
export class LoginComponent implements OnInit {
  ...
  public selecionarEscola(event: Event): void {
    this.esc_id = parseInt((<HTMLInputElement>event.target).value, 10);
    const escolasArray: Array<Object> = Object.keys(this.escolas).map(
      i => this.escolas[i],
    );
  ...
  public listarEscolas(event: Event): void {
    const email = (<HTMLInputElement>event.target).value;
        ...
        this.accessService
          ...
          .then((response: Response) => {
            ...
            if (this.status_ativo_usuario === 1) {
              ...
              const str_menus = JSON.stringify(this.menus);
              const str_dados_escola = JSON.stringify(this.dados_escola);
              const str_escopo_perfil = JSON.stringify(this.escopo_perfil);
  ...
  public logarUsuarioAnonimamenteFirebase(): void {
    ...
    auth.signInAnonymously().then((userCredencials: firebase.auth.UserCredential) => {
      ...
      const dados_escola = JSON.parse(Utils.decriptAtoB(localStorage.getItem('dados_escola'),
