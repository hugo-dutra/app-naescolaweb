...
export class ImportarNotaBoletimComponent implements OnInit {
  ...
  public carregarDisciplinas(): void {
    ...
    this.disciplinaService.listar().toPromise().then((response: Response) => {
  ...
  public gerarModeloPlanilhaBoletim(): void {
    let modeloPlanilhaImportarNotasBoletim: ExcelProper.Workbook = new Excel.Workbook();
    ...
    var rows = [abreviturasDisciplinas];
    ...
    let rowFaltasNotas: ExcelProper.Row = modeloPlanilhaImportarNotasBoletim.worksheets[0].getRow(3);
    ...
    for (let i = this.disciplinas.length * 2 + 3; i > 0; i--) {
      let valorCelula = modeloPlanilhaImportarNotasBoletim.worksheets[0].getRow(2).getCell(i).value;
      if (valorCelula != "") {
    ...
    let identificadorArquivoTurma = this.stringTurmaSelecionada.replace('-', '_');
  ...
  public enviarArquivo(event: any): void {
    ...
    let resultadosEstudantes = new Array<any>();
    let arrayOfDisciplinas = new Array<string>();
