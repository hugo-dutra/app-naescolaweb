...
export class GerenciarAplicativoComponent implements OnInit {
  ...
  public gravarEntradasManuaisDoAplicativo(messagesFirebase: Array<MessageFirebase>): void {
    ...
    for (let i = 0; i < messagesFirebase.length; i++) {
      ...
      this.firebaseService.gravarEntradasManuaisFirebaseFirestore(messageFirebase).then((response: Response) => {
        ...
        this.EnviarPushEntradaManual(topicoPush, tituloPush, messageFirebase.firebase_dbkey, messagesFirebase.length, i);
      }).catch((erro: Response) => {
        this.tratarErro(erro);
      })
    }
  ...
  public EnviarPushEntradaManual(topico: string, titulo: string, firebase_dbkey: string, total: number, atual: number): void {
    this.feedbackUsuario = `Enviando notificação ${atual} de ${total}, aguarde...`;
    if (firebase_dbkey != "") {
      this.firebaseService
        .enviarPushFirebase(topico, titulo)
  ...
  public inserirEntradaManualDoAplicativo(): void {
    ...
    this.portariaService.inserirEntradasDoAplicativo(por_id, this.arrayDeEntradasPeloAplicativo).toPromise().then((response: Response) => {
      this.firebaseService.atualizarStatusEntradasManuaisDepoisDeSincronizar(this.inep, this.documentosParaAtualizar).then((response: string) => {
        this.feedbackUsuario = undefined;
