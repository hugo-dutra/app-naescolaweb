...
export class ListarComunicadoDiversoComponent implements OnInit {
  ...
  public selecionarStatusMensagem(event: Event): void {
    this.status_mensagem = parseInt((<HTMLInputElement>event.target).value, 10);
  }

  public gravarDados(event: Event) {
    const nome = (<HTMLInputElement>event.target).name;
  ...
  public ordenarColuna(campo: string): void {
    if (!this.decrescente) {
      const retorno = this.arrayOfComunicados.sort(function (a, b) {
        if (a[campo] < b[campo]) {
          return 1;
        }
        if (a[campo] > b[campo]) {
